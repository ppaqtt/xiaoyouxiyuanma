...
pygame.display.set_caption("🎮 Python 小游戏合集 - 游戏启动器")
...
games_data = {
    ...
    "街机复刻": [
        {"name": "贪吃蛇", "file": "snake_game.py", "desc": "经典街机游戏"},
        {"name": "贪吃蛇3D", "file": "snake_3d.py", "desc": "伪3D透视效果"},
        {"name": "贪吃蛇双人", "file": "snake_2p.py", "desc": "双人对战模式"},
        {"name": "贪吃蛇变种", "file": "snake_variants.py", "desc": "多种模式"},
        {"name": "2048", "file": "game_2048.py", "desc": "数字合并"},
        {"name": "俄罗斯方块", "file": "tetris.py", "desc": "经典方块游戏"},
        {"name": "打砖块", "file": "breakout.py", "desc": "弹球打砖块"},
...
def draw_categories():
    ...
    for i, category in enumerate(categories):
        ...
        if i == current_category:
            pygame.draw.rect(screen, category["color"], (x, y, 140, 40), border_radius=5)
            pygame.draw.rect(screen, WHITE, (x, y, 140, 40), 3, border_radius=5)
        else:
            pygame.draw.rect(screen, DARK_GRAY, (x, y, 140, 40), border_radius=5)
        
        text = font_small.render(category["name"], True, WHITE)
        text_rect = text.get_rect(center=(x + 70, y + 20))
...
def draw_games():
    ...
    for i in range(visible_count):
        ...
        if is_selected:
            pygame.draw.rect(screen, YELLOW, (x - 5, y - 5, 280, 150), 3, border_radius=10)
        
        pygame.draw.rect(screen, (60, 60, 80), (x, y, 270, 140), border_radius=10)
...
def draw_search():
    pygame.draw.rect(screen, (40, 40, 60), (20, HEIGHT - 70, WIDTH - 40, 50), border_radius=8)
    search_text = font_medium.render(f"搜索: {search_query}", True, WHITE)
    screen.blit(search_text, (40, HEIGHT - 55))
...
def draw_scrollbar():
    ...
    if total_games > 12:
        scroll_height = HEIGHT - 180
        thumb_height = max(40, (12 / total_games) * scroll_height)
        thumb_y = 140 + (scroll_offset / total_games) * scroll_height
        
        pygame.draw.rect(screen, DARK_GRAY, (WIDTH - 25, 140, 15, scroll_height))
...
while running:
    ...
    title_text = font_large.render("🎮 Python 小游戏合集 - 游戏启动器", True, YELLOW)
    screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, 5))
    
    count_text = font_small.render(f"共 {len(all_games)} 个游戏", True, WHITE)
    screen.blit(count_text, (WIDTH - 150, 15))
